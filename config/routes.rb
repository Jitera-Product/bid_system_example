...
Rails.application.routes.draw do
  ...
  namespace :api do
    ...
    # The new code for product_categories with search action is nested incorrectly
    # inside another namespace :api block. It should be directly under the first
    # namespace :api block. The following is the corrected code.
    resources :product_categories, only: %i[index create show update] do
      collection do
        get 'search', to: 'product_categories#search'
      end
    end
