  ...
  resource_owner_from_credentials do |_routes|
    ...
    if params[:scope].present?
      ...
      if params[:email].present? && params[:password].present?
        ...
        when 'admins'
          resource = Admin.authenticate?(params[:email], params[:password])
...
Doorkeeper::OAuth::Token.class_eval do
  class << self
    def authenticate(request, *methods)
      ...
      access_token.revoke_previous_refresh_token! if access_token.present? && access_token.use_refresh_token?
      access_token
    end
  end
end
