  ...
  path '/oauth/token' do
    post 'Oauth Token' do
      ...
      parameter name: :params, in: :body, schema: {
        type: :object,
        properties: {
          client_id: { type: :string, example: Doorkeeper::Application.find_by(scopes: 'admins')&.uid.to_s },
          client_secret: { type: :string, example: Doorkeeper::Application.find_by(scopes: 'admins')&.secret.to_s },
          password: { type: :string, example: 'password' },
          email: { type: :string, example: 'email@domaiin.com' },
          scope: { type: :string, example: 'adminsusers any one based on login needed' },
      ...
      response '200', 'request sent' do
        ...
        let(:resource) { create(:admins, password: password) }
