class Api::AdminsController < Api::BaseController
  ...
  def moderate_content
    ...
    if current_resource_owner.role != 'admin'
      render json: { error: 'Only admins can moderate content' }, status: :unauthorized
      return
    end
    content = params[:content_type].capitalize.constantize.find(params[:content_id])
    if params[:action] == 'delete'
      content.destroy
      render json: { message: 'Content deleted successfully' }, status: :ok
    else
      content.update(content: params[:new_content])
      render json: { message: 'Content updated successfully', content_id: content.id }, status: :ok
    end
  end
end
