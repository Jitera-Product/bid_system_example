class User < ApplicationRecord
  ...
  def kyc_incomplete!
    begin
      self.update!(kyc_status: 'Incomplete')
    rescue => e
      Rails.logger.error "Failed to update KYC status for user #{self.id}: #{e.message}"
      false
    end
  end
  def notify_kyc_status_change
    # Here you can implement the logic to send a notification to the user
    # about the change in their KYC status and the feature restrictions.
  end
  class << self
