...
class PaymentMethodService::Index
  ...
  def primary_equal
    ...
    @records = if records.is_a?(Class)
                 PaymentMethod.where('primary = ?', params.dig(:wallets, :primary))
               else
  ...
  def method_equal
    ...
    @records = if records.is_a?(Class)
                 PaymentMethod.where('method = ?', params.dig(:wallets, :method))
               else
  ...
  def self.call(params, _current_user = nil)
    new(params, _current_user).execute
  end
end
